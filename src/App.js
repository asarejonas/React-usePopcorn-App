    ...
    function () {
      async function fetchMovie() {
        try {
          ...
          if (!res.ok) throw new Error("Something went fetching the movie");

          const data = await res.json();

          if (data.Response === "False")
            throw new Error("Movie not found in Data Base!");
        ...
        <Box>
          {selectedId ? (
            ...
            <>
              ...
              <WatchedMoviesList
                watched={watched}
                onDeletWatched={handleDeletWatchedMovie}
              />
            </>
          )}
        </Box>
...
function MovieDetails({ selectedId, onCloseMovie, onAddWatched, watched }) {
  ...
  function handleAdd() {
    const newWatchedMovie = {
      ...
      runtime: Math.round(Number(runtime.split(" ").at(0))),
      userRating,
    };
  ...
  return (
    <li>
      ...
      <div>
        ...
        <p>
          <span>🌟</span>
          <span>{Math.round(movie.userRating)}</span>
        </p>
        <p>
          <span>⏳</span>
